...
for e in eccentricities:
    ...
    print(f"Running Verlet (Apoapsis) for e = {e}")
    ...
    plt.plot(r[:,0], r[:,1])
    plt.axis("equal")
    plt.title(f"Verlet Apo Orbit e={e}")
    ...
    plt.title(f"Verlet Apo Fractional Energy Error e={e}")
    ...
    results.append([
        e, dt, orbits, len(t),
        final_error, energy_range, L_dev
    ])

    del r, v, t, E, L

df = pd.DataFrame(results, columns=[
    "eccentricity", "dt", "orbits", "steps",
    "final_fractional_energy_error",
    "energy_range",
    "angular_momentum_deviation"
])

df.to_csv(os.path.join(data_path, "verlet_apo_summary.csv"), index=False)

print("Verlet Apo complete with full structure.")